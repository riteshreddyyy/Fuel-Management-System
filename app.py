...
def reports():
    ...
    try:
        # 1. Join Query
        JOIN_QUERY = """
            SELECT T.Transaction_ID, E.Name AS Employee_Name, P.Pump_Number, F.Name AS Fuel_Type, T.Total_Amount
        ...
        # 2. Aggregate Query
        AGGREGATE_QUERY = """
            SELECT F.Name AS Fuel_Type, SUM(T.Total_Amount) AS Total_Revenue, COUNT(T.Transaction_ID) AS Number_of_Sales
